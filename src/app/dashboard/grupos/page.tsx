...
export default function GroupsPage() {
    ...
    const handleToggleDestination = useCallback(async (group: Group) => {
        if (group.isDestination) {
            // Removing destination — ask for confirmation
    ...
    return (
        <div className="space-y-10 pb-10">
            ...
            <div role="tablist" aria-label="Tipo de grupo" className="flex gap-4 p-1 bg-white/5 border border-white/10 rounded-2xl w-fit">
                <button
                    role="tab"
                    aria-selected={tab === 'origin'}
                    aria-controls="tab-panel-grupos"
            ...
            <div className="glass-card p-4 rounded-2xl flex items-center gap-4 group relative overflow-hidden">
                ...
                <Search className="w-5 h-5 text-muted ml-2 group-focus-within:text-primary transition-colors" aria-hidden="true" />
                <input
                    type="text"
                            ...
                            return (
                                <div key={group.id} className="glass-card rounded-[2.5rem] p-8 space-y-6 hover:translate-y-[-4px] transition-all duration-300 group shadow-2xl relative overflow-hidden">
                                    ...
                                    <div className="relative z-10">
                                        <h3 className="font-black text-xl text-white tracking-tight truncate" title={group.name}>{group.name}</h3>
                                        <div className="flex items-center gap-5 mt-2">
                                            <span className="text-[10px] text-muted font-bold flex items-center gap-1.5 uppercase tracking-wider">
                                                <Users className="w-3.5 h-3.5 text-primary" aria-hidden="true" /> {group.members}
                                            </span>
                                            <span className="text-[10px] text-muted font-bold flex items-center gap-1.5 uppercase tracking-wider">
                                                <MessageSquare className="w-3.5 h-3.5 text-secondary" aria-hidden="true" /> {group.lastMsg}
                                            </span>
                                        </div>
                                    </div>
                                    ...
                                    <div className="pt-4 flex gap-3 relative z-10">
                                        <button
                                            ...
                                            aria-label={group.isDestination ? `Remover ${group.name} dos grupos destino` : `Tornar ${group.name} um grupo destino`}
                                            className="flex-grow py-3.5 bg-white/5 border border-white/10 rounded-2xl text-[10px] font-black uppercase tracking-widest hover:bg-white/10 transition-all shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                                        >
                                        ...
                                        <button
                                            ...
                                            className={cn(
                                                "p-3.5 rounded-2xl transition-all border shadow-lg disabled:opacity-50 disabled:cursor-not-allowed",
                                                group.monitored
                    ...
                    <div className="glass-card rounded-[3rem] p-20 flex flex-col items-center justify-center text-center space-y-8 relative overflow-hidden group">
                        ...
                        <div className="flex flex-col sm:flex-row gap-4 relative z-10">
                            ...
                            <Link
                                ...
                                className="px-8 py-4 bg-white/5 border border-white/10 text-muted font-black text-xs uppercase tracking-widest rounded-2xl hover:bg-white/10 transition-all text-center"
                            >
                                Conectar WhatsApp
                            </Link>
                        </div>
